...
// Java에서는 단일 상속만 지원함
// 다중 상속에서는 발생할 수 있는 다이아몬드 문제가 발생하지 않도록 하기 위함
...
class SalesMan extends Human {
	...
	// 생성자, 반환타입 없음, 클래스명과 동일하게, this 사용 
	SalesMan(String name, int age, int sales) {
		super(name, sales);
		this.sales = sales;
		// 부모 생성자 호풀은 반드시 현재 생성자의 첫번째 구문으로 작성해야 함
		// super(name);
	}
	
	// 오버라이딩 : 하위 클래스에서 상위 클래스의 메서드를 '재정의' 하는 것
	// 규칙:
	// 1. 상위 클래스에서 선언된메서드와 메서드 이름, 매개변수의 조합이 같아야 한다.
	// 2. 상위 클래스에서 선왼된 메서드가 final로 선언되면 재정의가 불가능
